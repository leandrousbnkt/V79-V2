        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 200:
                    ...
                    return content
                elif response.status_code in [503, 451, 429, 403]:
                    logger.warning(f"⚠️ Jina Reader retornou status {response.status_code} para {url}")
                    # Para esses status, tenta fallback imediatamente
                    return self._fallback_extraction(url)
                else:
                    logger.warning(f"⚠️ Jina Reader retornou status {response.status_code} para {url}")
                    # Para outros status, continua tentando
    ...
    def _extract_with_readability(self, url: str) -> Optional[str]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                content_data = response.text if hasattr(response, 'text') else response.content.decode('utf-8', errors='ignore')
                doc = Document(content_data)
                content = doc.summary()
